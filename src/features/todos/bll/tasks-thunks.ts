...
>('tasks/updateTask', async (param: UpdateTaskThunkType<UpdateTaskDomainModelType>, thunkAPI) => {
  thunkAPI.dispatch(setAppStatus({ status: 'progress' }))
  const currentTask = thunkAPI
    .getState()
    .tasks[param.todolistId].items.find((el) => el.id === param.taskId)
  if (!currentTask) {
    return handleAsyncServerNetworkError('task not found', thunkAPI)
  }
  const updateModel: UpdateTaskRequestType = {
